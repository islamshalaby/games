...
class AddressController extends Controller
{
    ...
    {
        $this->middleware('auth:api' , ['except' => ['getdeliveryprice' , 'getareas', 'getAllAreas', 'getGovernorates']]);
    }
        ...
        if (isset($visitor['id'])) {
            ...
            if (count($userAddress) > 0) {
                for($i = 0; $i < count($userAddress); $i++){
                    if($request->lang == 'en'){
                        $userAddress[$i]['area_name'] = Area::find($userAddress[$i]['area_id'])['title_en'];
                    }else{
                        $userAddress[$i]['area_name'] = Area::find($userAddress[$i]['area_id'])['title_ar'];
                    }
                    if($user->main_address_id == $userAddress[$i]['id']){
                        $userAddress[$i]['is_default'] =  true;
                    }else{
                        $userAddress[$i]['is_default'] =  false;
