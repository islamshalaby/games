...
class OrderController extends Controller
{
    ...
    public function orderDetails(Request $request, Order $order) {
        ...
        if (empty($data['order']['address']->title)) {
            $data['order']['address']->title = '__';
        }
        if (empty($data['order']['address']->gaddah)) {
            $data['order']['address']->gaddah = '__';
        }
        if (empty($data['order']['address']->building)) {
            $data['order']['address']->building = '__';
        }
        if (empty($data['order']['address']->apartment_number)) {
            $data['order']['address']->apartment_number = '__';
        }
        if (empty($data['order']['address']->floor)) {
            $data['order']['address']->floor = '__';
        }
    ...
    public function updateOrderStatus(Request $request, Order $order) {
        ...
        // $notification->save();
        // $user = Visitor::select('id','fcm_token', 'user_id')->where('user_id', $order->user_id)->where('fcm_token' ,'!=' , null)->get();
    ...
    public function orderAddressDetails(Request $request, UserAddress $address) {
        ...
        if (empty($data['address']->title)) {
            $data['address']->title = '__';
        }
        if (empty($data['address']->gaddah)) {
            $data['address']->gaddah = '__';
        }
        if (empty($data['address']->building)) {
            $data['address']->building = '__';
        }
        if (empty($data['address']->apartment_number)) {
            $data['address']->apartment_number = '__';
        }
        if (empty($data['address']->floor)) {
            $data['address']->floor = '__';
        }
        if (empty($data['address']->extra_details)) {
            $data['address']->extra_details = '__';
        }
