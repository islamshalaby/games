...
class NotificationController extends AdminController{
    ...
    public function show(){
        $data['notifications'] = Notification::where('type', 1)->orderBy('id' , 'desc')->get();
    ...
    public function send(Request $request){
        ...
        $arrays = array_chunk($fcm_tokens, 900);
		for ($n = 0; $n < count($arrays); $n ++) {
			$notificationss = APIHelpers::send_notification($notification->title , $notification->body , $the_image , null , $arrays[$n]);
		};    
    ...
    public function resend(Request $request){
        ...
        $users_tokens = Visitor::where('fcm_token' ,'!=' , null)->select('fcm_token')->pluck('fcm_token')->toArray();
		$arrays = array_chunk($users_tokens, 900);
		for ($n = 0; $n < count($arrays); $n ++) {
			$notificationss = APIHelpers::send_notification($notification->title , $notification->body , $notification->image , null , $arrays[$n]);
		}
