...
class HomeSectionsController extends AdminController{
    ...
    public function fetchData(Request $request) {
        if ($request->element == 1 || $request->element == 5) {
            $rows = Ad::where('place', 1)->orderBy('id' , 'desc')->get();
    ...
    public function addPost(Request $request) {
        $post = $request->all();
        $home_section_post = $request->except('ads', 'minislider', 'offers', 'categories');
        ...
        if ((isset($post['minislider']) && count($post['minislider']) > 0)) {
            foreach($post['minislider'] as $ad) {
                $home_element['element_id'] = $ad;
                $home_element['home_id'] = $home_section['id'];
                HomeElement::create($home_element);
            }
