        ...
        <div class="statbox widget box box-shadow">
            <div class="col-lg-12 filtered-list-search mx-auto">
                <div class="btn-group" role="group" aria-label="Basic example">
                    <a href="{{ route('orders.store.index') }}" type="button" class="btn btn-{{ strpos($url,'show') !== false || strpos($url,'fetchbydate') !== false ? 'light' : 'dark' }}">{{ __('messages.all_orders') }}</a>
                    <a href="{{ route('orders.store.filter', 1) }}" type="button" class="btn btn-{{ request()->segment(count(request()->segments())) == 1 ? 'light' : 'dark' }}">{{ __('messages.in_progress_orders') }}</a>
                    <a href="{{ route('orders.store.filter', 3) }}" type="button" class="btn btn-{{ request()->segment(count(request()->segments())) == 3 ? 'light' : 'dark' }}">{{ __('messages.canceled_orders') }}</a>
                    <a href="{{ route('orders.store.filter', 2) }}" type="button" class="btn btn-{{ request()->segment(count(request()->segments())) == 2 ? 'light' : 'dark' }}">{{ __('messages.delivered_orders') }}</a>
                </div>
            </div>
            <div class="widget-content widget-content-area">
                <div class="row">
                    <div class="form-group col-md-3">
                        <form id="areaForm" method="" action="{{ strpos($url,'filter') !== false ? '' : route('orders.store.fetchbyarea') }}">
                    ...
                    <div class="form-group col-md-6">
                        <form id="dateForm" method="" action="{{ strpos($url,'filter') !== false ? '' : route('orders.store.fetchbydate') }}">
                    ...
                    <div class="form-group col-md-3">
                        <form id="paymentForm" method="" action="{{ strpos($url,'filter') !== false ? '' : route('orders.store.fetchbypayment') }}">
                        ...
                        @foreach ($data['orders'] as $order)
                            <tr>
                                ...
                                <td class="hide_col text-center">
                                    @if($order->status == 1)
                                    <a style="margin-bottom: 5px" href="{{ route('orders.store.items.action', [$order->id, 3]) }}" onclick='return confirm("{{ __('messages.are_you_sure') }}");' class="btn btn-sm btn-danger hide_col">
                                        {{ __('messages.cancel_order') }}
                                    </a>
                                    <a href="{{ route('orders.store.items.action', [$order->id, 2]) }}" onclick='return confirm("{{ __('messages.are_you_sure') }}");' class="btn btn-sm btn-success hide_col">
                                ...
                                <td class="text-center blue-color hide_col"><a href="{{ route('orders.store.details', $order->id) }}" ><i class="far fa-eye"></i></a></td>
                                <td class="text-center blue-color hide_col"><a href="{{ route('orders.store.invoice', $order->id) }}" ><i class="far fa-eye"></i></a></td>
                            </tr>
                            <?php $i ++ ?>
                        @endforeach
                    </tbody>
