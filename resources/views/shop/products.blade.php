            ...
            $.ajax({
                ...
                success : function (data) {
                    ...
                    data.forEach(function(element) {
                        ...
                        var permition = [],
                            detailsLink = "/store-panel/products/details/" + element.id,
                            editLink = "/store-panel/products/edit/" + element.id,
                            deleteLink = "/store-panel/products/delete/" + element.id,
                            dinar = "{{ __('messages.dinar') }}",
                            visibilityStatus = "{{ __('messages.visible') }}",
                            hideShoProduct = "{{ __('messages.hide_product') }}",
                            hideShowLink = "/store-panel/products/hide/" + element.id + "/" + 1

                            if (element.hidden == 1) {
                                hideShoProduct = "{{ __('messages.show_product') }}"
                                hideShowLink = "/store-panel/products/hide/" + element.id + "/" + 0
                        ...
                        var rowNode = dTbls.row.add( [
                            `${i}`,
                            `<img src="https://res.cloudinary.com/dezsm0sg7/image/upload/w_50,q_50/v1581928924/${ (element.images[0].image) ? element.images[0].image : '' }"  />`,
                            `${elementName}`,
                            `${cat}`,
...
<div id="badgeCustom" class="col-lg-12 mx-auto layout-spacing">
    <div class="statbox widget box box-shadow">
        <div class="widget-content widget-content-area">
            ...
            @if($data['expire'] == 'no')
            <a class="btn btn-primary" href="/store-panel/products/show?expire=soon">{{ __('messages.expired_soon') }}</a>
            @endif

            @if($data['expire'] == 'soon')
            <a class="btn btn-primary" href="/store-panel/products/show">{{ __('messages.return_all_products') }}</a>
            @endif            
        </div>
                ...
                <table id="html5-extension" class="table table-hover non-hover">
                    ...
                    <tbody>
                        ...
                        @foreach ($data['products'] as $product)
                            <tr>
                                <td><?=$i;?></td>
                                <td class="hide_col"><img src="https://res.cloudinary.com/dezsm0sg7/image/upload/w_50,q_50/v1581928924/{{ isset($product->images[0]->image) ? $product->images[0]->image : '' }}"  /></td>
                                ...
                                <td class="hide_col">
                                    <div class="btn-group">
                                        ...
                                        <div class="dropdown-menu" aria-labelledby="dropdownMenuReference5" style="will-change: transform;">
                                          <a class="dropdown-item" href="{{ route('products.store.details', $product->id) }}">{{ __('messages.details') }}</a>

                                            <a class="dropdown-item" href="{{ route('products.store.edit', $product->id) }}">{{ __('messages.edit') }}</a>
                                          
                                            <a class="dropdown-item"  onclick='return confirm("{{ __('messages.are_you_sure') }}");' href="{{ route('products.store.delete', $product->id) }}">{{ __('messages.delete') }}</a>
                                             
                                          <div class="dropdown-divider"></div>
                                          @if($product->hidden == 0)
                                          <a class="dropdown-item" onclick='return confirm("{{ __('messages.are_you_sure') }}");' href="{{ route('products.store.visibility.status', [$product->id, 1]) }}">{{ __('messages.hide_product') }}</a>
                                          @else
                                          <a class="dropdown-item" onclick='return confirm("{{ __('messages.are_you_sure') }}");' href="{{ route('products.store.visibility.status', [$product->id, 0]) }}">{{ __('messages.show_product') }}</a>
                                          @endif
        ...
        {{-- <div class="paginating-container pagination-solid">
            <ul class="pagination">
                ...
                @for($i = 1 ; $i <= $data['categories']->lastPage(); $i++ )
                    <li class="{{ $data['categories']->currentPage() == $i ? "active" : '' }}"><a href="/store-panel/categories/show?page={{$i}}">{{$i}}</a></li>               
                @endfor
