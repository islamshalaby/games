        ...
        <div class="statbox widget box box-shadow">
            <div class="col-lg-12 filtered-list-search mx-auto">
                <div class="btn-group" role="group" aria-label="Basic example">
                    ...
                    <a href="{{ route('orders.filter', 4) }}" type="button" class="btn btn-{{ request()->segment(count(request()->segments())) == 3 ? 'light' : 'dark' }}">{{ __('messages.canceled_orders') }}</a>
            ...
            <div class="widget-content widget-content-area">
                <div class="row">
                    ...
                    <div class="form-group col-md-6">
                        <form id="dateForm" method="" action="{{ strpos($url,'filter') !== false ? '' : route('orders.fetchbysubnumber') }}">
                            <div class="form-group mb-4">
                                <div class="row">
                                    <div class="col-md-8">
                                        <label for="sub_number">{{ __('messages.sub_order_number') }}</label>
                                        <input required type="text" name="sub_number" class="form-control" id="sub_number" >
                                    </div>
                                    <div class="col-md-4">
                                        <input style="margin-top: 39px;" type="submit" value="{{ __('messages.submit') }}" class="btn btn-primary">
                                    </div>
                                </div>
                            </div>
                        ...
                        @foreach ($data['orders'] as $order)
                            <tr>
                                ...
                                {{-- <td class="hide_col text-center">
                                    @if($order->status == 1)
                                    <a style="margin-bottom: 5px" href="{{ route('orders.action', [$order->id, 3]) }}" onclick='return confirm("{{ __('messages.are_you_sure') }}");' class="btn btn-sm btn-danger hide_col">
                                        {{ __('messages.cancel_order') }}
                                    </a>
                                    <a href="{{ route('orders.action', [$order->id, 3]) }}" onclick='return confirm("{{ __('messages.are_you_sure') }}");' class="btn btn-sm btn-success hide_col">
                                        {{ __('messages.order_delivered') }}
                                    </a>
                                    @endif
                                    @if($order->status == 3)
                                    <a style="margin-bottom: 5px" href="{{ route('orders.action', [$order->id, 4]) }}" onclick='return confirm("{{ __('messages.are_you_sure') }}");' class="btn btn-sm btn-danger hide_col">
                                        {{ __('messages.cancel_order') }}
                                    </a>
                                    @endif
                                </td> --}}
                                <td class="text-center blue-color hide_col"><a href="{{ route('orders.details', $order->id) }}" ><i class="far fa-eye"></i></a></td>
                                <td class="text-center blue-color hide_col"><a href="{{ route('webview.invoice', $order->id) }}" ><i class="far fa-eye"></i></a></td>
                            </tr>
                            <?php $i ++ ?>
                        @endforeach
                    </tbody>
