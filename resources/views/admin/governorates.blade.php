                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <a href="{{ route('areas.details', $area->id) }}" target="_blank">
                                        <span style="margin-bottom: 5px" class="badge outline-badge-info">{{ App::isLocale('en') ? $area->title_en : $area->title_ar }}</span>
                                    </a>
                                    @endforeach
                                    @endif
                                </td>
                                <td class="text-center blue-color"><a href="{{ route('areas.governorates.details', $governorate->id) }}" ><i class="far fa-eye"></i></a></td>
                                @if(Auth::user()->update_data) 
                                    <td class="text-center blue-color" ><a href="{{ route('areas.governorates.edit', $governorate->id) }}" ><i class="far fa-edit"></i></a></td>
                                @endif
                                @if(Auth::user()->delete_data) 
                                    <td class="text-center blue-color" ><a onclick='return confirm("{{ __('messages.are_you_sure') }}");' href="{{ route('areas.delete', $governorate->id) }}" ><i class="far fa-trash-alt"></i></a></td>
                                @endif                                
                                <?php $i++; ?>
                            </tr>
                        @endforeach
